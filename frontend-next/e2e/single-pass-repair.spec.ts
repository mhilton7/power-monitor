...
const bill = {
  ...
  fields: [
    { id: 'field-1', field_key: 'plan_name', effective_value: 'DOMESTIC', confidence: 'high', page_number: 1 },
    { id: 'field-2', field_key: 'total_usage_kwh', effective_value: '951.000', confidence: 'high', page_number: 1 },
    { id: 'field-3', field_key: 'tier_1_rate', effective_value: '0.30000000', confidence: 'high', page_number: 3 },
  ],
  conflicts: [],
  blocking_warnings: [],
  normalized: {},
...
const ratePlans = {
  plans: [{
    ...
    latest_version: { id: 'version-1', version: 2, pricing_model: 'time_of_use' },
  }],
}
...
async function mockRepairServer(page: Page, configured = false, options: MockOptions = {}) {
  ...
  const observed: ObservedRequests = { retired: false, removed: false, restored: false, sourceAdded: false }
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (pathname === `/api/v1/admin/utility-bill-imports/${bill.id}/publish-and-assign`) return route.fulfill({ json: { status: 'active' } })
    if (pathname === `/api/v1/admin/utility-bill-imports/${bill.id}/import-billing-cycle`) return route.fulfill({ json: { status: 'imported' } })
    ...
    if (pathname === '/api/v1/admin/rate-plans/plan-1/dependencies') {
      return route.fulfill({ json: { permanent_draft_deletion_eligible: false, removal_blocked: false, preservation: { versions: true, source_evidence: true } } })
    }
    ...
    const responses: Record<string, unknown> = {
      '/api/v1/auth/session': {
        ...
        user: { id: 'owner-1', email: 'owner@example.test', display_name: 'Home Owner', roles: ['admin'], permissions: ['rates.view', 'rates.manage_custom', 'rates.manage_sources', 'rates.assign'], all_sites: true, site_ids: [] },
      },
      '/api/v1/sites': [home],

...
export function ProtectedChangeDialog({
  ...
  return (
    <div className="modal-backdrop protected-change-backdrop">
      ...
      >
        <header>
          <div><small>Security confirmation</small><h2 id="protected-change-title">Confirm this protected change</h2></div>
        ...
        <div className="setup-body form-grid single">
          <div className="protected-change-intro" id="protected-change-description">
            ...
            <p>{mfaEnabled ? 'Enter either your current password or a six-digit MFA code.' : 'Enter your current password to save these high-risk permission changes.'}</p>
          </div>
        ...
        <footer>
          ...
          <button className="button primary" type="submit" disabled={!confirmationReady || confirmChange.isPending}>{confirmChange.isPending ? 'Confirming…' : 'Confirm and save'}</button>
        </footer>
      </form>
    </div>
  )

...
const FirmwarePage = lazy(() => import('./pages/FirmwarePage').then((module) => ({ default: module.FirmwarePage })))
...
function ProtectedApp({ session }: { session: Session }) {
  ...
  const canOperate = session.user?.roles.some((role) => role === 'admin' || role === 'operator') ?? false
  return (
    <Layout session={session}>
      <Suspense fallback={<LoadingState label="Opening this workspace…" />}>
        <Routes>
        <Route path="/" element={<DashboardPage canOperate={canOperate} />} />
        ...
        <Route path="/enrollment" element={canOperate ? <EnrollmentPage /> : <Navigate to="/" replace />} />
        <Route path="/firmware" element={canOperate ? <FirmwarePage /> : <Navigate to="/" replace />} />
        <Route path="/reports" element={<ReportsPage />} />
        <Route path="/admin" element={session.user?.roles.includes('admin') ? <AdminPage /> : <Navigate to="/" replace />} />

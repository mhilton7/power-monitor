          ...
          >
            <Suspense fallback={<LoadingState label="Opening this workspace…" />}>
              <Routes>
                ...
                <Route path="/billing/rate-plans" element={<ScopedRouteBoundary scope="Billing workspace"><InWorkspace session={session} workspaceId="billing"><Guard session={session} permission="rates.view"><RatesPage canManage={canManageRates} canImportBills={canManageBills} canRemove={hasPermission(session, 'rates.remove')} canRestore={hasPermission(session, 'rates.restore')} /></Guard></InWorkspace></ScopedRouteBoundary>} />

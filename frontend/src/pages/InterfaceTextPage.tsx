...
export function InterfaceTextPage({ canManage }: { canManage: boolean }) {
  ...
  return <>
    ...
    {canManage && <footer className="sticky-action-bar"><span>{dirtyCount} unpublished local change(s){draft.data?.exists ? ` · saved draft revision ${draft.data.draft_revision}${draft.data.previewed_revision === draft.data.draft_revision ? ' · previewed' : ' · preview required'}` : ''}</span><div><button className="button ghost" disabled={!draft.data?.exists || discard.isPending} onClick={() => { discard.mutate() }}>Discard draft</button><button className="button ghost" onClick={() => { void exportText(true) }}><Download size={15} />Export draft</button><CanonicalAction id="interface_text.save_draft" surface="sticky_footer"><button className="button secondary" disabled={saveDraft.isPending} onClick={() => { saveDraft.mutate() }}><Save size={15} />Save draft</button></CanonicalAction><CanonicalAction id="interface_text.publish" surface="sticky_footer"><button className="button primary" disabled={!draft.data?.exists || draft.data.previewed_revision !== draft.data.draft_revision} title={draft.data?.exists && draft.data.previewed_revision !== draft.data.draft_revision ? 'Preview this draft revision before publishing' : undefined} onClick={() => { setShowPublish(true) }}><Send size={15} />Publish</button></CanonicalAction></div></footer>}
    {showPublish && <ActionScope scopeKey="interface-text-publish-dialog"><div className="modal-backdrop" role="presentation"><section className="confirm-dialog" role="dialog" aria-modal="true" aria-label="Publish interface text"><header><div><span className="eyebrow">Immutable revision</span><h2>Publish interface text?</h2></div><button className="icon-button" onClick={() => { setShowPublish(false) }} aria-label="Close publication confirmation"><X /></button></header><p>This makes the validated draft visible to dashboard users and the safe public login endpoint. Internal routes, API identifiers, and permission codes will not change.</p><dl className="detail-list"><div><dt>Base revision</dt><dd>{draft.data?.base_revision}</dd></div><div><dt>Draft revision</dt><dd>{draft.data?.draft_revision}</dd></div><div><dt>Changed fields</dt><dd>{Object.keys(draft.data?.values ?? {}).length}</dd></div></dl>{publish.error && <div className="form-error"><strong>Publication failed</strong><span>{errorMessage(publish.error)}</span></div>}<footer><button className="button secondary" onClick={() => { setShowPublish(false) }}>Cancel</button><CanonicalAction id="interface_text.publish" surface="dialog"><button className="button primary" disabled={publish.isPending} onClick={() => { publish.mutate() }}>{publish.isPending ? 'Publishing…' : 'Confirm and publish'}</button></CanonicalAction></footer></section></div></ActionScope>}

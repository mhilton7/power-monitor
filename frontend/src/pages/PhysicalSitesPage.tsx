  ...
  return (
    <div className="modal-backdrop" role="presentation">
      <section className="site-wizard" role="dialog" aria-modal="true" aria-label="Add physical site">
        ...
        <ol className="site-wizard-steps" aria-label="Site creation steps">{wizardSteps.map((label, index) => <li className={index === step ? 'active' : index < step ? 'complete' : ''} aria-current={index === step ? 'step' : undefined} key={label}><span aria-hidden="true">{index < step ? <Check size={13} /> : index + 1}</span><span className="site-wizard-step-label">{label}</span></li>)}</ol>

...
export function NetworkPolicyPanel({ sites, initialSiteId }: { sites: Site[]; initialSiteId?: string }) {
  ...
  const runtime = useQuery({ queryKey: ['network-runtime'], queryFn: () => api<NetworkRuntime>('/api/v1/admin/network/runtime') })
  const observed = useQuery({ queryKey: ['observed-devices', siteId], queryFn: () => api<ObservedDevice[]>(`/api/v1/admin/network/observed-devices?site_id=${siteId}`), enabled: Boolean(siteId) })
  ...
  return <>
    <Panel title="Sensor network policy" eyebrow="Explicit per-site controls">
    ...
    {policy && <div className="network-settings-grid"><Panel title="Private CIDR rules" eyebrow="Canonical server validation" actions={<button className="button ghost" onClick={() => { suggest.mutate(); }}>Add current private network</button>}>
    ...
    <Panel title="Observed signed device addresses" eyebrow="Heartbeat evidence">

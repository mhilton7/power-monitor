...
NOTIFICATION_CATALOG: dict[str, NotificationCatalogEntry] = {
    ...
    "storage_sequence_reconciling": _entry(
        "storage",
        "warning",
        "{resource} is online, but storage continuity is not ready",
        "The server accepted the heartbeat, but the sensor has not yet saved the required sequence floor to microSD.",
        "Live monitoring remains available. Durable history pauses until the sequence journal is verified.",
        "Keep the sensor powered while local reconciliation retries.",
        "Review the required next sequence and the journal failure evidence.",
        "Use the sensor storage controls only if reconciliation does not complete automatically.",
        automatic_recovery="The sensor retries the bounded journal operation locally without network backoff.",
        action_label="Open sensor details",
        action_target="/settings/sensors",
        action_permissions=("storage.view",),
    ),
    "storage_sequence_continuity_restored": _entry(
        "storage",
        "info",
        "{resource} detected a new or empty microSD card",
        "The sensor restored its local sequence floor above server history; no history was deleted and no re-enrollment is required.",
        "New durable readings resume at the reported next safe sequence.",
        "No action is required.",
        "Keep the sensor online until its first new interval is synchronized.",
        automatic_recovery="This notice resolves after the first durable record is created.",
        action_label="Open sensor details",
        action_target="/settings/sensors",
        action_permissions=("storage.view",),
    ),
    "storage_cursor_regression": _entry(
        "storage",
        "critical",
        "Server sequence state is behind {resource}",
        "Power Monitor will not lower the sensor cursor automatically because that could reuse sequence numbers or overwrite protected history.",
        "Server connectivity remains available, but durable synchronization is blocked for integrity review.",
        "Preserve the card, NVS, and server database while reviewing cursor evidence.",
        "Do not factory-reset or re-enroll the sensor.",
        "Restore the authoritative server cursor or complete a documented recovery.",
        action_label="Open sensor details",
        action_target="/settings/sensors",
        action_permissions=("storage.view",),
    ),
    "storage_pressure_notice": _entry(
        "storage",
        "info",
        "{resource} microSD storage is 80% full",
        "The sensor reports the first graduated local-storage pressure threshold.",
        "Automatic storage protection remains enabled and no immediate action is required.",
        "Review the remaining-space estimate.",
        "Keep the sensor connected so acknowledged history can be reclaimed safely.",
        "Plan a larger card if the estimate is shorter than expected.",
        automatic_recovery="The sensor removes only closed, verified, server-acknowledged segments allowed by its retention policy.",
        action_label="Review sensor storage",
        action_target="/settings/sensors",
        action_permissions=("storage.view",),
    ),
    "storage_pressure_warning": _entry(
        "storage",
        "warning",
        "{resource} microSD storage is running low",
        "The sensor reports less than the configured warning amount of free storage.",
        "Eligible synchronized history will be reclaimed while unacknowledged readings remain protected.",
        "Keep server synchronization working.",
        "Review the acknowledged sequence and pending-reading count.",
        "Run safe cleanup or prepare a replacement card if available space continues falling.",
        automatic_recovery="Acknowledgement-aware cleanup runs through the sensor StorageTask.",
        action_label="Review sensor storage",
        action_target="/settings/sensors",
        action_permissions=("storage.view",),
    ),
    "storage_pressure_critical": _entry(
        "storage",
        "critical",
        "{resource} has less than 5% microSD space remaining",
        "The sensor entered critical storage pressure and queued safe cleanup.",
        "New historical intervals are at risk if protected history prevents enough reclamation.",
        "Keep the sensor connected to drain its synchronization backlog.",
        "Verify that safe cleanup is completing.",
        "Prepare a larger replacement card if free space remains critical.",
        automatic_recovery="Only verified, closed, acknowledged segments are eligible for automatic removal.",
        action_label="Review sensor storage",
        action_target="/settings/sensors",
        action_permissions=("storage.view",),
    ),
    "storage_pressure_emergency": _entry(
        "storage",
        "critical",
        "{resource} microSD storage is at emergency capacity",
        "The sensor has reached its emergency reserve or full-card state.",
        "Live measurement can continue, but durable History may develop visible gaps if no safe space can be reclaimed.",
        "Restore synchronization immediately.",
        "Run safe cleanup after confirming the server acknowledgement cursor.",
        "Prepare and install a larger microSD card if cleanup cannot restore the reserve.",
        action_label="Review sensor storage",
        action_target="/settings/sensors",
        action_permissions=("storage.view",),
    ),
    "storage_cleanup_blocked": _entry(
        "storage",
        "critical",
        "{resource} cannot safely reclaim enough storage",
        "Cleanup found no eligible segment that is both verified and acknowledged by the server.",
        "Power Monitor preserves those readings; new historical intervals may be lost if storage reaches zero.",
        "Restore server synchronization.",
        "Review the unacknowledged-reading count and acknowledgement cursor.",
        "Use the safe card-replacement workflow if synchronization cannot recover in time.",
        action_label="Review sensor storage",
        action_target="/settings/sensors",
        action_permissions=("storage.view",),
    ),
    "storage_write_reserve_unavailable": _entry(
        "storage",
        "critical",
        "{resource} could not save a historical interval",
        "No safe write reserve remained on the microSD card.",
        "Live measurements may continue, but History coverage is incomplete and the gap is not replaced with zero.",
        "Restore synchronization and allow safe cleanup to run.",
        "Inspect the cleanup result and protected-byte counts.",
        "Prepare a replacement microSD card if the write reserve does not recover.",
        action_label="Review sensor storage",
        action_target="/settings/sensors",
        action_permissions=("storage.view",),
    ),
    "storage_interval_dropped": _entry(
        "storage",
        "critical",
        "{resource} lost one or more durable history intervals",
        "The sensor exhausted its bounded storage queue while the microSD card could not accept safe writes.",
        "Live measurement can continue, but the affected History range remains an explicit gap and is never replaced with zero.",
        "Restore server synchronization and safe free space.",
        "Review the first and last affected interval timestamps.",
        "Prepare a larger replacement card if safe cleanup cannot maintain the write reserve.",
        action_label="Review sensor storage",
        action_target="/settings/sensors",
        action_permissions=("storage.view",),
    ),
